...
res <- read.delim("/lyceum/cle1g21/smORFs/TCGA/BRCA/Outputs/DEGS_lncRNAs_BRCA.csv", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
View(res)
...
write.table(resOrdered_nuORFs, file = "/lyceum/cle1g21/smORFs/TCGA/BRCA/Outputs/lncRNAs_nuORFs_matched_BRCA.txt", sep = "\t", row.names = TRUE)
...
write.csv(top15_downreg, "/lyceum/cle1g21/smORFs/TCGA/BRCA/Outputs/top15upreg_lncRNA_matched_nuORFs.csv")
...
ggsave("/lyceum/cle1g21/smORFs/TCGA/BRCA/Outputs/Figures/volcano_lncRNAs_nuORFs_3.png", plot = volcano, width = 10, height = 8, dpi = 300, bg = "white")
