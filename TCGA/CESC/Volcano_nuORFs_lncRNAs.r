# Match new lncRNA DEGs from Volcano_DEGS_lncRNAs.r to nuORFs
library(tibble)
library(ggplot2)
library(ggrepel)

res <- read.delim("/lyceum/cle1g21/smORFs/TCGA/CESC/Outputs/DEGS_lncRNAs_CESC.csv", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
View(res)
...
write.table(resOrdered_nuORFs, file = "/lyceum/cle1g21/smORFs/TCGA/CESC/Outputs/lncRNAs_nuORFs_matched_CESC.txt", sep = "\t", row.names = TRUE)

res <- read.delim("/lyceum/cle1g21/smORFs/TCGA/CESC/Outputs/lncRNAs_nuORFs_matched_CESC.txt", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
View(res)
nrow(res)
...
write.csv(top15_downreg, "/lyceum/cle1g21/smORFs/TCGA/CESC/Outputs/top15upreg_lncRNA_matched_nuORFs.csv")
...
ggsave("/lyceum/cle1g21/smORFs/TCGA/CESC/Outputs/Figures/volcano_lncRNAs_nuORFs_2.png", plot = volcano, width = 8, height = 6, dpi = 300, bg = "white")
