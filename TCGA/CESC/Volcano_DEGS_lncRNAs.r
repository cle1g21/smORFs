# DEGs filtered to include only lncRNAs - write DEGS_lncRNAs_CESC.csv
library(org.Hs.eg.db)
library(biomaRt)

# read in DESeq2 results for BRCA dataset
res <- read.delim("/lyceum/cle1g21/smORFs/TCGA/CESC/Outputs/DESeq2results_CESC_filtered_variants.txt", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
View(res)
...
saveRDS(gene_labels, file = "/lyceum/cle1g21/smORFs/TCGA/CESC/Outputs/gene_labels.rds")

write.table(res_lncRNA, file = "/lyceum/cle1g21/smORFs/TCGA/CESC/Outputs/DEGS_lncRNAs_CESC.csv", sep = "\t", row.names = TRUE)
...
ggsave("/lyceum/cle1g21/smORFs/TCGA/CESC/Outputs/Figures/volcano_DEGS_only_lncRNA_1.png", plot = volcano, width = 8, height = 6, dpi = 300, bg = "white")
