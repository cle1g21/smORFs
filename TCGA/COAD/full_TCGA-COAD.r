...
cts <- read.csv("/mainfs/ddnb/TCGA_data/full_TCGA-COAD_matrix.csv")
#View(cts)
...
write.table(res, file = "/lyceum/cle1g21/smORFs/TCGA/DESeq2results_COAD.txt", sep = "\t", row.names = TRUE)
...
res <- read.table("/lyceum/cle1g21/smORFs/TCGA/COAD/DESeq2results_COAD.txt", header = TRUE, row.names = 1, sep = "\t", stringsAsFactors = FALSE)
View(res)
...
write.table(res, "/lyceum/cle1g21/smORFs/TCGA/COAD/DESeq2results_COAD_filtered_variants.txt", sep = "\t", quote = FALSE, row.names = TRUE)
