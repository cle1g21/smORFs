...
res <- read.delim("/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/DEGS_lncRNAs_KICH.csv", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
View(res)
...
write.table(resOrdered_nuORFs, file = "/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/lncRNAs_nuORFs_matched_KICH.txt", sep = "\t", row.names = TRUE)

res <- read.delim("/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/lncRNAs_nuORFs_matched_KICH.txt", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
View(res)
nrow(res)
...
write.csv(top15_downreg, "/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/top15upreg_lncRNA_matched_nuORFs.csv")
...
ggsave("/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/Figures/volcano_lncRNAs_nuORFs_1.png", plot = volcano, width = 8, height = 6, dpi = 300, bg = "white")
