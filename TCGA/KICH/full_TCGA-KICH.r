...
cts <- read.csv("/mainfs/ddnb/TCGA_data/full_TCGA-KICH_matrix.csv")
#View(cts)
...
# Run DESeq2
dds <- DESeq(dds) # 4 minutes to run
saveRDS(dds, file = paste0("/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/dds.rds"))
...
write.table(res, file = "/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/DESeq2results_KICH.txt", sep = "\t", row.names = TRUE)
...
res <- read.table("/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/DESeq2results_KICH.txt", header = TRUE, row.names = 1, sep = "\t", stringsAsFactors = FALSE)
View(res)
...
write.table(res, "/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/DESeq2results_KICH_filtered_variants.txt", sep = "\t", quote = FALSE, row.names = TRUE)
...
write.table(resOrdered_nuORFs, file = "/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/nuORFs_matched_KICH.txt", sep = "\t", row.names = TRUE)
...
top15_upreg <- read.csv("/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/top15upreg_lncRNA_matched_nuORFs.csv", header = TRUE)

top15_upreg_genes <- top15_upreg$Gene_ID
top15_upreg_label <- top15_upreg$label

# Save plot as PNG
png("/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/Figures/Plot_Counts/plot_count_top15_upregulated_1.png", width = 800, height = 600)
...
for (i in 1:length(top15_upreg_genes)) {
    ...
    file_name <- paste0("/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/Figures/Plot_Counts/plot_count_", gene_symbol, ".png")
