...
res <- read.delim("/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/DESeq2results_KICH_filtered_variants.txt", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
View(res)
...
saveRDS(gene_labels, file = "/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/gene_labels.rds")

write.table(res_lncRNA, file = "/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/DEGS_lncRNAs_KICH.csv", sep = "\t", row.names = TRUE)
...
ggsave("/lyceum/cle1g21/smORFs/TCGA/KICH/Outputs/Figures/volcano_DEGS_only_lncRNA_1.png", plot = volcano, width = 8, height = 6, dpi = 300, bg = "white")
