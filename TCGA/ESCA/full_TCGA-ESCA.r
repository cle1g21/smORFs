...
cts <- read.csv("/mainfs/ddnb/TCGA_data/full_TCGA-ESCA_matrix.csv")
View(cts)
...
# Run DESeq2
dds <- DESeq(dds) # 6 minutes to run
saveRDS(dds, file = paste0("/lyceum/cle1g21/smORFs/TCGA/ESCA/Outputs/dds.rds"))
...
write.table(res, file = "/lyceum/cle1g21/smORFs/TCGA/ESCA/Outputs/DESeq2results_ESCA.txt", sep = "\t", row.names = TRUE)
...
res <- read.table("/lyceum/cle1g21/smORFs/TCGA/ESCA/OutputsDESeq2results_ESCA.txt", header = TRUE, row.names = 1, sep = "\t", stringsAsFactors = FALSE)
View(res)
...
write.table(res, "/lyceum/cle1g21/smORFs/TCGA/ESCA/Outputs/DESeq2results_ESCA_filtered_variants.txt", sep = "\t", quote = FALSE, row.names = TRUE)
...
write.table(resOrdered_nuORFs, file = "/lyceum/cle1g21/smORFs/TCGA/ESCA/Outputs/nuORFs_matched_ESCA.txt", sep = "\t", row.names = TRUE)
...
top15_upreg <- read.csv("/lyceum/cle1g21/smORFs/TCGA/ESCA/Outputs/top15upreg_lncRNA_matched_nuORFs.csv", header = TRUE)
nuORFs_matched <- read.csv("/lyceum/cle1g21/smORFs/TCGA/ESCA/Outputs/nuORFs_matched_ESCA.txt", header = TRUE)

top15_upreg_genes <- top15_upreg$Gene_ID
top15_upreg_label <- top15_upreg$label

# Save plot as PNG
png("/lyceum/cle1g21/smORFs/TCGA/ESCA/Outputs/Figures/Plot_Counts/ENSG00000174365.png", width = 800, height = 600)
...
for (i in 1:length(top15_upreg_genes)) {
    ...
    file_name <- paste0("/lyceum/cle1g21/smORFs/TCGA/ESCA/Outputs/Figures/Plot_Counts/plot_count_", gene_symbol, ".png")
